...
#			  default: ${PYTHON_PKGNAMEPREFIX}build>0:python/py-build@${PY_FLAVOR}
#
...
#			  default: ${PYTHON_PKGNAMEPREFIX}installer>0:python/py-installer@${PY_FLAVOR}
#
...
.    if ${PYTHON_VER} == 2.7
BUILD_DEPENDS+=		${PYTHON_PKGNAMEPREFIX}setuptools44>0:python/py-setuptools44@${PY_FLAVOR}
RUN_DEPENDS+=		${PYTHON_PKGNAMEPREFIX}setuptools44>0:python/py-setuptools44@${PY_FLAVOR}
.    else
BUILD_DEPENDS+=		${PYTHON_PKGNAMEPREFIX}setuptools>=63.1.0:python/py-setuptools@${PY_FLAVOR}
RUN_DEPENDS+=		${PYTHON_PKGNAMEPREFIX}setuptools>=63.1.0:python/py-setuptools@${PY_FLAVOR}
.    endif
.  endif
...
PEP517_BUILD_DEPEND?=	${PYTHON_PKGNAMEPREFIX}build>0:python/py-build@${PY_FLAVOR}
...
PEP517_INSTALL_DEPEND?=	${PYTHON_PKGNAMEPREFIX}installer>0:python/py-installer@${PY_FLAVOR}

# nose support
.  if defined(_PYTHON_FEATURE_NOSE)
TEST_DEPENDS+=	${PYTHON_PKGNAMEPREFIX}nose>=0:python/py-nose@${PY_FLAVOR}
.  endif

# nose2 support
.  if defined(_PYTHON_FEATURE_NOSE2)
TEST_DEPENDS+=	${PYTHON_PKGNAMEPREFIX}nose2>=0:python/py-nose2@${PY_FLAVOR}
.  endif

# pytest support
.  if defined(_PYTHON_FEATURE_PYTEST)
TEST_DEPENDS+=	${PYTHON_PKGNAMEPREFIX}pytest>=7,1:python/py-pytest@${PY_FLAVOR}
.  elif defined(_PYTHON_FEATURE_PYTEST4)
TEST_DEPENDS+=	${PYTHON_PKGNAMEPREFIX}pytest4>=4.6,1:python/py-pytest4@${PY_FLAVOR}
.  endif
.  if defined(_PYTHON_FEATURE_PYTEST) || defined(_PYTHON_FEATURE_PYTEST4)
...
TEST_DEPENDS+=	${PYTHON_PKGNAMEPREFIX}unittest2>=0:python/py-unittest2@${PY_FLAVOR}
.  endif
...
PYGAME=		${PYTHON_PKGNAMEPREFIX}game>0:python/py-game@${PY_FLAVOR}
PYNUMPY=	${PYTHON_PKGNAMEPREFIX}numpy>=1.16,1<1.25,1:math/py-numpy@${PY_FLAVOR}

.  if defined(_PYTHON_FEATURE_DISTUTILS)
.    if ${PYTHON_MAJOR_VER} < 3
PY_SETUPTOOLS=	${PYTHON_PKGNAMEPREFIX}setuptools44>0:python/py-setuptools44@${PY_FLAVOR}
.    else
#PY_SETUPTOOLS=	${PYTHON_PKGNAMEPREFIX}setuptools58>0:python/py-setuptools58@${PY_FLAVOR}
PY_SETUPTOOLS=	${PYTHON_PKGNAMEPREFIX}setuptools>0:python/py-setuptools@${PY_FLAVOR}
.    endif
.  else
PY_SETUPTOOLS=	${PYTHON_PKGNAMEPREFIX}setuptools>0:python/py-setuptools@${PY_FLAVOR}
.  endif
...
PY_MERCURIAL=	${PYTHON_PKGNAMEPREFIX}mercurial>=5.9:python/mercurial@${PY_FLAVOR}
.  else
PY_MERCURIAL=	mercurial>=5.9:python/mercurial@${PY_FLAVOR}
.  endif

CMAKE_ARGS+=	-DBOOST_PYTHON_SUFFIX:STRING=${PYTHON_SUFFIX}
PY_BOOST_LIB=	boost_python${PYTHON_SUFFIX}
PY_BOOST=	lib${PY_BOOST_LIB}.so:python/boost-python-libs@${PY_FLAVOR}
