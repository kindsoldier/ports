# $FreeBSD: head/Mk/Uses/python.mk 504981 2019-06-23 19:21:24Z sunpoet $
#
...
BUILD_DEPENDS+=		${PYTHON_PKGNAMEPREFIX}setuptools>0:python/py-setuptools@${PY_FLAVOR}
RUN_DEPENDS+=		${PYTHON_PKGNAMEPREFIX}setuptools>0:python/py-setuptools@${PY_FLAVOR}
.endif
...
PYNUMPY=	${PYTHON_PKGNAMEPREFIX}numpy>0:python/py-numpy@${PY_FLAVOR}

# Common Python modules that can be needed but only for some versions of Python.
.if ${PYTHON_REL} < 3500
PY_TYPING=	${PYTHON_PKGNAMEPREFIX}typing>=3.6.4:python/py-typing@${PY_FLAVOR}
.else
PY_TYPING=
.endif

.if ${PYTHON_REL} < 3400
PY_ENUM34=	${PYTHON_PKGNAMEPREFIX}enum34>0:python/py-enum34@${PY_FLAVOR}
PY_ENUM_COMPAT=	${PYTHON_PKGNAMEPREFIX}enum-compat>0:python/py-enum-compat@${PY_FLAVOR}
PY_PATHLIB=	${PYTHON_PKGNAMEPREFIX}pathlib>0:python/py-pathlib@${PY_FLAVOR}
.else
PY_ENUM34=
PY_ENUM_COMPAT=
PY_PATHLIB=	
...
.if ${PYTHON_REL} < 3300
PY_IPADDRESS=	${PYTHON_PKGNAMEPREFIX}ipaddress>0:python/py-ipaddress@${PY_FLAVOR}
.else
PY_IPADDRESS=
.endif

.if ${PYTHON_REL} < 3200
PY_FUTURES=	${PYTHON_PKGNAMEPREFIX}futures>=3.2:python/py-futures@${PY_FLAVOR}
.else
PY_FUTURES=
.endif
