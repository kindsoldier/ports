...
LIB_DEPENDS+=	libpq.so.${PGSQL${PGSQL_VER_NODOT}_LIBVER}:data/postgresql${PGSQL_VER_NODOT}
.endif
...
_USE_PGSQL_DEP_docs=	postgresql${PGSQL_VER_NODOT}>0
_USE_PGSQL_DEP_pgtcl=	${LOCALBASE}/lib/pgtcl/pkgIndex.tcl
_USE_PGSQL_DEP_plperl=	postgresql${PGSQL_VER_NODOT}>0
_USE_PGSQL_DEP_plpython=postgresql${PGSQL_VER_NODOT}>0
_USE_PGSQL_DEP_pltcl=	postgresql${PGSQL_VER_NODOT}>0
_USE_PGSQL_DEP_server=	postgresql
.    if defined(WANT_PGSQL)
.      for depend in ${_USE_PGSQL_DEP}
.        if ${WANT_PGSQL:M${depend}}
BUILD_DEPENDS+=	${_USE_PGSQL_DEP_${depend}}:data/postgresql${PGSQL_VER_NODOT}
RUN_DEPENDS+=	${_USE_PGSQL_DEP_${depend}}:data/postgresql${PGSQL_VER_NODOT}
.        elif ${WANT_PGSQL:M${depend}\:*}
BUILD_DEPENDS+=	${NONEXISTENT}:data/postgresql${PGSQL_VER_NODOT}:${WANT_PGSQL:M${depend}\:*:C,^[^:]*\:,,}
.        endif
.      endfor
.    endif
