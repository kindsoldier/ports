...
+++ ./src/network/ssl/qsslsocket_openssl_symbols_p.h	2019-12-24 00:25:23.229095000 +0200
@@ -206,17 +206,23 @@
...
+const int q_BIO_free(const BIO *a);
...
+int q_PEM_write_bio_DSAPrivateKey(const BIO *a, DSA *b, const EVP_CIPHER *c, unsigned char *d,
                                   int e, pem_password_cb *f, void *g);
-int q_PEM_write_bio_RSAPrivateKey(BIO *a, RSA *b, const EVP_CIPHER *c, unsigned char *d,
+int q_PEM_write_bio_RSAPrivateKey(const BIO *a, RSA *b, const EVP_CIPHER *c, unsigned char *d,
                                   int e, pem_password_cb *f, void *g);
 #endif
...
+int q_PEM_write_bio_DSA_PUBKEY(const BIO *a, DSA *b);
+int q_PEM_write_bio_RSA_PUBKEY(const BIO *a, RSA *b);
...
+#else
+int q_OPENSSL_init_ssl(uint64_t opts, OPENSSL_INIT_SETTINGS *settings);
...
+#else
+int q_OPENSSL_init_crypto(uint64_t opts, OPENSSL_INIT_SETTINGS *settings);
